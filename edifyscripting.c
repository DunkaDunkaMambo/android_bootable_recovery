...
Value* RestoreFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (strcmp(args2[i], "noboot") == 0)
            ...
            restorecache = 0;
        else if (strcmp(args2[i], "noemmc") == 0)
            restoreemmc = 0;
        else if (strcmp(args2[i], "nosd-ext") == 0)
            restoresdext = 0;
        else if (strcmp(args2[i], "noimei") == 0)
            restoreimei = 0;
    }
    ...
    if (0 != nandroid_restore(path, restoreboot, restorecust, restoresystem, restoredata, restorecache, restoreemmc, restoresdext, restoreimei, 0)) {
        free(path);
        return StringValue(strdup(""));
    }
